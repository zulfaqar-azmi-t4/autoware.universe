...
{
  if (!common_data_ptr->is_data_available() || !common_data_ptr->lanes_available()) {
  ...
  const auto & path = common_data_ptr->path_from_centerline_ptr->current;
...
double calc_lane_change_buffer(
  const CommonDataPtr & common_data_ptr, const lanelet::ConstLanelets & lanes)
{
  if (!common_data_ptr->is_data_available() || lanes.empty()) {
  ...
  return calc_lane_change_buffer(lc_param_ptr, shift_intervals);
}
...
//   return calc_lane_change_buffer(lc_param_ptr, {shift_intervals.front()});
// }
