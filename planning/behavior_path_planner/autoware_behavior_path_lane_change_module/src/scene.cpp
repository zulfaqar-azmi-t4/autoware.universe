...
void NormalLaneChange::update_lanes(const bool is_approved)
{
  ...
  *common_data_ptr_->lanes_polygon_ptr = create_lanes_polygon(common_data_ptr_);
  ...
  if (!common_data_ptr_->path_from_centerline_ptr) {
    common_data_ptr_->path_from_centerline_ptr =
      std::make_shared<lane_change::PathFromCenterLine>();
  }

  common_data_ptr_->path_from_centerline_ptr->current =
    route_handler_ptr->getCenterLinePath(current_lanes, 0.0, std::numeric_limits<double>::max());
  common_data_ptr_->path_from_centerline_ptr->target =
...
bool NormalLaneChange::isLaneChangeRequired()
{
  ...
  const auto ego_dist_to_target_start =
    calculation::calc_ego_dist_to_lanes_start(common_data_ptr_, target_lanes);
...
bool NormalLaneChange::is_near_regulatory_element() const
{
  ...
  const auto current_lane_change_buffer =
    calculation::calc_lane_change_buffer(common_data_ptr_, current_lanes);
  const auto dist_to_terminal_start =
    calculation::calc_ego_dist_to_terminal_end(common_data_ptr_) - current_lane_change_buffer;
...
TurnSignalInfo NormalLaneChange::get_terminal_turn_signal_info() const
{
  ...
  const auto current_lane_change_buffer =
    calculation::calc_lane_change_buffer(common_data_ptr_, get_current_lanes());

  const double buffer = current_lane_change_buffer +
  ...
  const lanelet::ConstLanelets & lanelets, PathWithLaneId & path)
{
  ...
  const auto current_lane_change_buffer =
    calculation::calc_lane_change_buffer(common_data_ptr_, lanelets);
  ...
  double stopping_distance = distance_to_terminal - current_lane_change_buffer - stop_point_buffer;
  ...
  if (distance_to_ego_lane_obj < distance_to_terminal) {
    ...
    const auto stopping_distance_for_obj =
      distance_to_ego_lane_obj - current_lane_change_buffer -
  ...
  const double threshold) const
{
  ...
  const auto distance_to_lane_change_end = std::invoke([&]() {
    ...
    return std::max(0.0, distance_to_end) - current_lane_change_buffer;
  });
...
bool NormalLaneChange::is_near_terminal() const
{
  ...
  const auto min_lc_dist_with_buffer =
    backward_buffer + current_lane_change_buffer + lc_param_ptr->lane_change_finish_judge_buffer;
  ...
  const lanelet::ConstLanelets & target_lanes, [[maybe_unused]] const Direction direction) const
{
  ...
  const auto next_lane_change_buffer =
    calculation::calc_lane_change_buffer(common_data_ptr_, target_lanes);
  ...
  if (
    ...
    lane_change_length + next_lane_change_buffer >
  ...
  for (const auto & prepare_duration : prepare_durations) {
    for (const auto & sampled_longitudinal_acc : longitudinal_acc_sampling_values) {
      ...
      const auto ego_dist_to_terminal_start =
        dist_to_end_of_current_lanes - current_lane_change_buffer;
      ...
      for (const auto & lateral_acc : sample_lat_acc) {
        ...
        if (
          !is_stuck && !utils::lane_change::passed_parked_objects(
                         ...
                         current_lane_change_buffer, lane_change_debug_.collision_check_objects)) {
          debug_print_lat(
  ...
  const lanelet::ConstLanelets & current_lanes, const lanelet::ConstLanelets & target_lanes) const
{
  ...
  const auto lane_changing_start_pose = autoware::motion_utils::calcLongitudinalOffsetPose(
    ...
    -(current_lane_change_buffer + next_lane_change_buffer + distance_to_terminal_from_goal));
  ...
  const auto target_segment = getTargetSegment(
    target_lanes, lane_changing_start_pose.value(), target_lane_length, current_lane_change_buffer,
  ...
  lane_change_info.length = LaneChangePhaseInfo{0.0, current_lane_change_buffer};
  ...
  const auto resample_interval = utils::lane_change::calcLaneChangeResampleInterval(
    current_lane_change_buffer, minimum_lane_changing_velocity);
  const auto target_lane_reference_path = utils::lane_change::getReferencePathFromTargetLane(
    route_handler, target_lanes, lane_changing_start_pose.value(), target_lane_length,
    current_lane_change_buffer, forward_path_length, resample_interval, is_goal_in_route,
    next_lane_change_buffer);
...
PathSafetyStatus NormalLaneChange::isApprovedPathSafe() const
{
  ...
  const auto has_passed_parked_objects = utils::lane_change::passed_parked_objects(
    common_data_ptr_, path, filtered_objects_.target_lane_leading, current_lane_change_buffer,
    debug_data);
...
bool NormalLaneChange::calcAbortPath()
{
  ...
  const auto lane_changing_end_pose_idx = std::invoke([&]() {
    ...
    const double s_end = std::max(
      lanelet::utils::getLaneletLength2d(reference_lanelets) - current_lane_change_buffer, 0.0);
  ...
  const lanelet::ConstLanelets & current_lanes, const double obstacle_check_distance) const
{
  ...
  const auto current_lane_change_buffer =
    calculation::calc_lane_change_buffer(common_data_ptr_, current_lanes);
  const double stop_point_buffer = lane_change_parameters_->backward_length_buffer_for_end_of_lane;
  const double terminal_judge_buffer = current_lane_change_buffer + stop_point_buffer + 1.0;
