...
struct MetricBasedSegment
{
  ...
  MetricBasedSegment() = default;
  MetricBasedSegment(
    double duration, double longitudinal_distance, double velocity,
...
struct SegmentBasedMetric
{
  double prepare{0.0};
  double lane_changing{0.0};

  SegmentBasedMetric() = default;
  SegmentBasedMetric(double prepare, double lane_changing)
  : prepare(prepare), lane_changing(lane_changing)
  {
  }
};
...
{
  SegmentBasedMetric duration;
  SegmentBasedMetric longitudinal_distance;
  SegmentBasedMetric velocity;
  SegmentBasedMetric longitudinal_acceleration;

  SegmentMetric() = default;
  SegmentMetric(MetricBasedSegment prepare_segment, MetricBasedSegment lane_changing_segment)
  : duration(prepare_segment.duration, lane_changing_segment.duration),
    longitudinal_distance(
      prepare_segment.longitudinal_distance, lane_changing_segment.longitudinal_distance),
    velocity(prepare_segment.velocity, lane_changing_segment.velocity),
    longitudinal_acceleration(
      prepare_segment.longitudinal_acceleration, lane_changing_segment.longitudinal_acceleration)
  {
  }

  SegmentMetric(
    double prepare_duration, double lane_changing_duration, double prepare_segment_lon_dist,
    double lane_changing_segment_lon_dist, double prepare_velocity, double lane_changing_velocity,
    double prepare_segment_lon_acc, double lane_changing_segment_lon_acc)
  : duration(prepare_duration, lane_changing_duration),
    longitudinal_distance(prepare_segment_lon_dist, lane_changing_segment_lon_dist),
    velocity(prepare_velocity, lane_changing_velocity),
    longitudinal_acceleration(prepare_segment_lon_acc, lane_changing_segment_lon_acc)
  {
  }

  [[nodiscard]] double sum_duration() const { return duration.prepare + duration.lane_changing; }

  [[nodiscard]] double sum_longitudinal_distance() const
  {
    return longitudinal_distance.prepare + longitudinal_distance.lane_changing;
  }
};
using SegmentMetrics = std::vector<SegmentMetric>;

struct Info
{
  SegmentMetric segment_metric{};
